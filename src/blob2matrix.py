...
def data_2_matrix(data, filters, stride_h, stride_w, padding_h, padding_w,*args):
    ...
    _, C, R, S = args[0][0],args[0][1],args[0][2],args[0][3]

    data_padded = np.pad(data, ((0, 0), (0, 0), (padding_h, padding_h), (padding_w, padding_w)), mode='constant')
    k, i, j = get_im2col_indices(data.shape, stride_h, stride_w, padding_h, padding_w,*args)

    cols = np.concatenate(data_padded[:, k, i, j], axis=1)
    return cols


def filter_matrix_2_orig(filter_2d, shape):
    """Reshape (K, C*R*S) filter into (K, C, R, S).
...
def data_matrix_to_orig(data_2d, data_shape, filter_shape, stride_h, stride_w, padding_h, padding_w):
...
def get_im2col_indices(x_shape, stride_h, stride_w, padding_h, padding_w,*args):
    N, C, H, W = x_shape
    _, _, R, S = args[0][0],args[0][1],args[0][2],args[0][3]
